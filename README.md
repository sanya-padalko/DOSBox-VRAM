# Вывод рамочки с регистрами в DOSBox

По нажатию определенной клавиши выводится рамочка с нынешниими значениями регистров DOSBox.

![Базовая рамочка](images/base_frame.gif)

***Базовый вид рамки***

## Описание

### 1. Установка программы-резидента

Для того, чтобы рамочка выводилось необходимо сохранить код программы в память DOSа. Чтобы это сделать, запустите ***reg_frame.com*** (он находится в папке ***frame09***).

![reg_frame](images/reg_frame.png)

```cmd
> reg_frame.com
```

Чтобы проверить, что резидент установился, нажмите ***Alt-F5***. Вы увидите ***Memory info***. В одной из строк в столбкие ***Program*** вы должны увидеть ***reg_fram.сom***. Если все сделано верно, в столбке ***Hooked vectors*** должны находиться ***08*** и ***09***.

![Memory info](images/memory-info.png)

Поздравляю, резидент установлен.

Чтобы удалить резидента из памяти, наведитесь в ***Memory Info*** на нужную строки и нажмите ***Release***.

### 2. Работа с рамочкой

Теперь при нажатии ***F11*** на экране появлятся рамка с актуальными значениями регистров.

![Пример рамочки](images/frame_example.png)

Чтобы рамочка исчезла, нажмите правый ***Shift***. Видеопамять под рамочкой будет иметь такой вид, какой она бы имела, если бы рамочки никогда не было.

![Без рамочки](images/no-frame.png)

### 3. Перемещение рамочки

Если рамочка включена, то используя стрелки её можно двигать.

Рассмотрим исходное расположение рамочки.

![До стрелочек](images/before_arrow.png)

Нажав несколько раз стрелочку вправо.

![После стрелочек вправо](images/after_arr1.png)

Теперь попробуем подвинуть её вниз.

![После стрелочек вниз](images/after_arr2.png)

За пределы окна ***DOS*** рамочка выйти не может ни по одной из границ.

![Angle frame](images/slice_frame.png)

### 4. Изименения стиля рамочки

Существуют два способа изменить стиль рамочки.

#### 1) Задать стиль рамочки при запуске ***reg_fram.com***

Чтобы рамочка рисовалась в базовом стиле нужно запустить ***reg_fram.com*** с аргументом **0** или без аргументов совсем:

```cmd
> reg_fram.com 0
```
или
```cmd
> reg_fram.com
```

![Обычный стиль](images/type0.png)


Чтобы вывести рамочку в некотором стиле, после ***reg_fram.com*** укажите индекс стиля, в котором вы бы хотели видеть рамочку:
```cmd
> reg_fram.com 1
```

![1-й стиль](images/type1.png)


```cmd
> reg_fram.com 2
```

![2-й стиль](images/type2.png)

Если вы хотите увидеть регистры из широко открытой пасти крокодила выбирайте 3-й стиль:
```cmd
> reg_fram.com 3
```
![3-й стиль](images/type3.png)

Всего у рамочки есть 4 заданных стиле. Поэтому, если задать индекс стиля 4 или больше, то рамочка будет составлена из символов, после таблицы стилей (в программе):

```cmd
> reg_frame.com 4
```

![4-й стиль](images/type4.png)

```cmd
> reg_frame.com 111
```

![111-й стиль](images/type111.png)

#### 2) Задать свой шрифт с помощью EVAFONT

Для этого перейдите в папку ***EVA***. В ней будут расположены файлы ***.fnt***, ***.com***, ***evafont.exe***. Чтобы сделать выбранный шрифт основным запустите ***.com***-файл. Так, один из доступных шрифтов:

```cmd
> wavy.com
```

![Wavy frame](images/wavy-frame.png)

Для того, чтобы вернуть шрифт к обычному, запустите ***base.com***:

```cmd
> base.com
```

С помощью ***EVAFONT*** вы можете сделать свой шрифт. Для этого запустите ***evafont.exe***:

```cmd
> evafont.exe
```

В открывшемся окне вы увидите следующее:

![evafont](images/evafont.png)

В панели ***File*** вы можете загрузить один из имеющихся стилей или сохранить свой. В панели ***Character -> Select*** выберите символ, который вы хотите изменить, а затем нажмите ***Edit*** и нарисуйте новый символ:

![apple](images/apple.png)

В панели ***Charaster*** вы можете сразу задать нынешний шрифт как основной.

### 5. Проверка правильности работы

Чтобы проверить, что регистры выводятся правильно, запустим тестовую программу. Откройте папку ***09h*** и запустите файл ***test-dri.com***:

```cmd
> test-dri.com
```

Чтобы увидеть рамочку нажмите ***F11***:

![Test 1](images/test1.png)

Во всех регистрах (кроме ***AL***, и ***IP***) после нажатия клавиши ***K*** изменят значение один раз, а дальше меняться не будут. На картинке вы можете увидеть ожидаемые значения регистров после нажатия ***K***:

![Test 2](images/test2.png)

Чтобы завершить тестирование нажмите ***Esc*** и вы сможете увидеть, что регистры снова начали актуально обновляться:

![Test 3](images/test3.png)

MIPT � [Padalko Aleksandt]
